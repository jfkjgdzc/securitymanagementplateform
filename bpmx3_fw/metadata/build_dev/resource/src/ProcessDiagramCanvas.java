...
public class ProcessDiagramCanvas {
	...
	protected static String copyrightTitle="本版本仅供学习使用.请勿用于商业用途.如需要商业用途,请联系我们购买授权,电话:400-0668-262; 网址:www.jee-soft.cn";
	protected static Map<Short, Color> colorsMap = new HashMap<Short, Color>();
	
	public ProcessDiagramCanvas(int width, int height) {
		this.canvasWidth = width;
		height=height+getCopyRightHeight(width);
	...
	public void drawWaterMark() {
		...
		this.g.setPaint(new Color(0.1f,0.1f,0.2f,0.3f));
		this.g.setFont(font);
		String tempCopyrightTitle=copyrightTitle;
		//对字符串进行拆分
		String[] nameAry=fitTextToWidthAndBreak(tempCopyrightTitle,(canvasWidth-minX)-10*font.getSize());
		int offsetY=font.getSize()*3/4+12;
		if(nameAry.length==1){
			this.g.drawString(tempCopyrightTitle,(canvasWidth -(StringUtil.getTotalSize(copyrightTitle)+10)*font.getSize())/2,canvasHeight-offsetY);
		}else if(nameAry.length==2){
			nameAry[0]="本版本仅供学习使用。请勿用于商业用途。如需要商业用途，请联系我们购买授权";
			nameAry[1]="电话：400-0668-262   网址：www.jee-soft.cn";
			this.g.drawString(nameAry[0],minX,canvasHeight-offsetY);
			this.g.drawString(nameAry[1],minX,canvasHeight-offsetY+font.getSize());
		}
	}
...
//		String text = fitTextToWidth(name, width);
	...
	public static Color getColor(String colorString){
		SysPropertyService sysPropertyService=new SysPropertyService();
	...
	//获取显示版权水印的高度
	public static int getCopyRightHeight(int canvasWidth){
		return (int) (13* Math.ceil(StringUtil.getTotalSize(copyrightTitle)/((float)canvasWidth/12)));
	}
	//获取显示版权水印的最大宽
	public static int getCopyRightWidth(){
		return StringUtil.getTotalSize(copyrightTitle)*14;
	}
}