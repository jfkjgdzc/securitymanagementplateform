...
public class AtsAttencePolicyController extends BaseController {
	...
	public void save(HttpServletRequest request, HttpServletResponse response, AtsAttencePolicy atsAttencePolicy) throws Exception {
		String resultMsg = null;
		try {
			if (atsAttencePolicy.getId() == null || atsAttencePolicy.getId() == 0) {
	...
	private void setAtsAttencePolicy(AtsAttencePolicy atsAttencePolicy) {
		if (BeanUtils.isNotEmpty(atsAttencePolicy.getAttenceCycle())) {
			AtsAttenceCycle atsAttenceCycle = atsAttenceCycleService.getById(atsAttencePolicy.getAttenceCycle());
		...
		if (BeanUtils.isNotEmpty(atsAttencePolicy.getWorkCalendar())) {
			AtsWorkCalendar atsWorkCalendar = atsWorkCalendarService.getById(atsAttencePolicy.getWorkCalendar());
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		List<AtsAttencePolicy> list = atsAttencePolicyService.getAll(new QueryFilter(request, "atsAttencePolicyItem"));
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		...
		if (BeanUtils.isEmpty(atsAttencePolicy)) {
			...
			setAtsAttencePolicy(atsAttencePolicy);
		return getAutoView().addObject("atsAttencePolicy", atsAttencePolicy).addObject("returnUrl", returnUrl);
	}
	...
	public ModelAndView selector(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return this.getAutoView().addObject("atsAttencePolicyList", list).addObject("isSingle", isSingle);
	}
}
