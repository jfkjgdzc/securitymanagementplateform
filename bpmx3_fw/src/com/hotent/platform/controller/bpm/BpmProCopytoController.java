...
public class BpmProCopytoController extends BaseController
{
	...
	public void finishDivert(HttpServletRequest request,
		...
		if (runId == 0 || BeanUtils.isEmpty(users)) {
			resultMessage = new ResultMessage(ResultMessage.Fail, "转发失败!");
		} else {
			...
			if (returnResult==-1) {
				resultMessage = new ResultMessage(ResultMessage.Fail, "转发失败!不能转发给自己!");
			}else if (returnResult==1) {
				resultMessage = new ResultMessage(ResultMessage.Success, "转发成功!");
			}else {
				resultMessage = new ResultMessage(ResultMessage.Fail, "转发失败!");
			} 
		}
