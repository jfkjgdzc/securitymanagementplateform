...
/**
 * 对象功能:自定义表单 控制器类 开发公司:软件有限公司 开发人员:xwy 创建时间:2011-12-22 11:07:56
 */
@Controller
...
public class BpmFormDefController extends BaseController {
	...
	public void delByFormKey(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (notDelArray.size() == 0) {
			...
			msg = new ResultMessage(ResultMessage.Fail, str);
		}
	...
	public ModelAndView parse(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		outHtml = BpmFormDefHtmlParser.parse(bpmFormDef.getHtml(), new ParserParam(data, permission));
		// 处理分页情况
		outHtml = bpmFormHandlerService.getTabHtml(bpmFormDef.getTabTitle(), outHtml);
	...
	@Action(description = "导出自定义表单XML", execOrder = ActionExecOrder.AFTER, detail = "导出自定义表单:" + "<#list StringUtils.split(formDefIds,\",\") as item>" + "<#assign entity=bpmFormDefService.getById(Long.valueOf(item))/>" + "【${entity.subject}】" + "</#list>")
