...
public class OutMailController extends BaseController
{
	...
	{	
		...
		try {
			//获取UID列表
			List<String> uidList= outMailService.getUIDBySetId(id);
			//读取邮件
			List<Mail> mailList= outMailService.getMailListBySetting(outMailUserSeting, uidList);
			//保存邮件
			outMailService.saveMail(mailList, id);
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception
	{
		...
		return getAutoView().addObject("outMail",outMail).addObject("returnUrl", returnUrl)
				.addObject("outMailUserSetingList",list);
	}
