...
public class PositionController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response,Position position) throws Exception
	{
		...
		try{
			if(position.getPosId()==null||position.getPosId()==0){
				if(positionService.isPoscodeUsed(position.getPosCode())){
					position.setPosId(UniqueIdUtil.genId());
					positionService.add(position);
					resultMsg=getText("添加成功","系统岗位表，实际是部门和职务的对应关系表");
					writeResultMessage(response.getWriter(),resultMsg,ResultMessage.Success);
				}else{
					resultMsg=getText("添加失败，岗位代码已被使用","系统岗位表，实际是部门和职务的对应关系表");
					writeResultMessage(response.getWriter(),resultMsg,ResultMessage.Fail);
				}
			}else{
				if(positionService.getPosCode(position.getPosId()).equals(position.getPosCode())){//若原岗位代码没有修改，可直接进行更新操作
					...
					if(positionService.isPoscodeUsed(position.getPosCode())){
						...
						resultMsg=getText("更新失败，岗位代码已被使用","系统岗位表，实际是部门和职务的对应关系表");
						writeResultMessage(response.getWriter(),resultMsg,ResultMessage.Fail);
					}
				}
			}
	...
	{	
	    ...
	    mv.addObject("action", "grade").addObject("orgAuth", orgAuth); 
		return mv;
	}
