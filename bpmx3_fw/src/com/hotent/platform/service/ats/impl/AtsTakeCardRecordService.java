...
public class AtsTakeCardRecordService implements IAtsTakeCardRecordService {
	...
	@Override
	public void getCardRecordByAccess(String filePath, String startDate, String endDate) throws Exception {
		ResultSet rs = null;
		Statement st = null;
		try {
			st = getAccessConnection(filePath);
			rs = st.executeQuery("SELECT u.BADGENUMBER BADGENUMBER,c.CHECKTIME CHECKTIME FROM USERINFO u ,CHECKINOUT c where u.USERID=c.USERID  AND c.CHECKTIME>=  #" + startDate + "# AND  c.CHECKTIME<=  #" + endDate + "# ");
			getList(rs);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException(e);
	...
	@Override
	public void getCardRecordByExcel(InputStream inputStream, String startDate, String endDate) {
		try {
	...
	@Override
	public void getCardRecordByText(InputStream inputStream, String filePath, String endDate) throws Exception {
		try {
			String encoding = "GBK";
	...
	public static void main(String[] args) throws Exception {
		Class.forName("net.ucanaccess.jdbc.UcanaccessDriver");
		Connection conn = DriverManager.getConnection("jdbc:ucanaccess://C:/att2000.mdb");
		Statement s = conn.createStatement();
		//		String startDate = "2015-02-01";
		//		String endDate = "2015-02-28";
		ResultSet rs = s.executeQuery("SELECT u.BADGENUMBER BADGENUMBER,c.CHECKTIME CHECKTIME FROM USERINFO u ,CHECKINOUT c where u.USERID=c.USERID ");
		while (rs.next()) {
