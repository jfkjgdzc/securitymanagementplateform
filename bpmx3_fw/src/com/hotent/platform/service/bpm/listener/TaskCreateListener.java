...
public class TaskCreateListener extends BaseTaskListener {
	...
	protected void execute(DelegateTask delegateTask,String actDefId,String nodeId)  {
		...
		//处理驳回到分发任务
		boolean handlerForkTaskReject = handlerForkTaskReject(actDefId, nodeId, delegateTask);
		if(handlerForkTaskReject) return;
	...
	private void handAssignUserFromDb(String actDefId,String nodeId,DelegateTask delegateTask){
		...
		if(processInstance!=null){
			...
			if(StringUtil.isEmpty(startUserId)){
				...
				startUserId = (String)executionEntity.getVariableLocal(BpmConst.StartUser);
			}
	...
	//处理驳回到分发任务的情况
	private boolean handlerForkTaskReject(String actDefId, String nodeId, DelegateTask delegateTask){
		ProcessCmd processCmd=TaskThreadService.getProcessCmd();
		if(processCmd!=null && !BpmConst.TASK_BACK.equals(processCmd.isBack())) return false;
		BpmNodeSet bpmNodeSet=bpmNodeSetDao.getByActDefIdNodeId(actDefId, nodeId);
		List<ForkTaskReject> forkTaskRejects = processCmd.getForkTaskRejects();
		if(BeanUtils.isEmpty(forkTaskRejects)) return false;
		bpmService.resurrecForkTasks((TaskEntity)delegateTask, forkTaskRejects);
		int size = forkTaskRejects.size();
		taskForkService.newTaskFork(delegateTask,bpmNodeSet.getJoinTaskName(), bpmNodeSet.getJoinTaskKey(), size);
		return true;
	}
	...
	private void assignUser(DelegateTask delegateTask, TaskExecutor taskExecutor){
		if(TaskExecutor.USER_TYPE_USER.equals(taskExecutor.getType())){
			...
			assignTaskExcutors(delegateTask,userList);
	...
	private void assignUser(DelegateTask delegateTask, List<TaskExecutor> executors){
		...
		if(executors.size()==1){
			...
			if(TaskExecutor.USER_TYPE_USER.equals(taskExecutor.getType())){
				...
				assignTaskExcutors(delegateTask,list);
			}
		}
		else{
			delegateTask.setAssignee(BpmConst.EMPTY_USER);
			...
			assignTaskExcutors(delegateTask,set);
	 ...
	 * @param collection
	 */
	private void assignTaskExcutors(DelegateTask delegateTask,Collection<TaskExecutor> collection){
		for(Iterator<TaskExecutor> it=collection.iterator();it.hasNext();){
