...
public class BpmFormDefService extends BaseService<BpmFormDef> {
	...
	public FormModel getNodeForm(ProcessRun processRun, String nodeId, Long userId, String ctxPath, Map<String, Object> variables, boolean isReadonly) throws Exception {
			...
			if (bpmFormDef != null && bpmFormDef.getFormKey().equals(formKey)) {
				...
				if (tableId > 0) {
					...
					if (BeanUtils.isNotEmpty(bpmFormTable)) {
						...
						//--->测试新解释器
						String formHtml = parseHtml(bpmFormDef, businessKey, instanceId, actDefId, nodeId, parentActDefId, false, false, isReadonly,false);
						//<---测试新解释器
		...
		if (BpmConst.OnLineForm.equals(bpmNodeSet.getFormType())) {
			...
			//--->测试新解释器
			String formHtml = parseHtml(bpmFormDef, bussinessKey, instanceId, actDefId, nodeId, parentActDefId, false, false, isReadonly,false);
			//<---测试新解释器
	 ...
	 * @since  1.0.0
	 */
	public String parseHtml(BpmFormDef bpmFormDef, String pkValue, String instanceId, String actDefId, String nodeId, String parentActDefId, boolean isReCalcute, boolean isPreView, boolean isReadOnly,boolean isCopy) throws Exception {
		String html = "";
		...
		BpmFormData data = bpmFormHandlerService.getBpmFormData(bpmFormDef.getBpmFormTable(), pkValue, instanceId, actDefId, nodeId, isReCalcute, isPreView,isCopy);
