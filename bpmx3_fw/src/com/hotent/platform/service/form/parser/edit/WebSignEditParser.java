 ...
 */
@Service
public class WebSignEditParser extends EditAbstractParser {
	...
	public Object parse(ParserParam param, Element element) {
		...
		if (!FieldRight.Y.getVal().equals(right)) {//非隐藏权限
			...
			inputE.attr("controltype", "webSign");
			inputE.attr("name", ename);
			inputE.val(val);
			inputE.attr("type","hidden");
			element.after(inputE.toString());
			String divId = "div_"+ename.replace(":", "_");
			element.after("<div class='webSign-div' id='"+divId+"' ></div>");
