 ...
 * 
 */
package com.hotent.platform.service.form.parser.table;
 ...
 */
@Service
public class WebSignTableParser extends TableAbstractParser {
	...
	public Object parse(ParserParam param, Element element) {
		...
		if (FieldRight.Y.getVal().equals(right)) {//隐藏权限
			...
			element.select("input").get(0).val(val);
			element.select("input").get(0).attr("right", right);
		}
		return null;
	}
}
