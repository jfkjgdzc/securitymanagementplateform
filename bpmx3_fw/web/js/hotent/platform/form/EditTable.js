...
var controlList = [ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},
                    ...
                    {key : '21',value : 'WebSign控件'},{key : '22',value : '图片展示控件'},{key : '23',value : '小数只读转百分比'}
                    ];
...
function setControlByType(fieldType){
	...
	$(controlList).each(function(i, d) {
		...
		if(fieldType==varchar_){
			...
			//数字
			if(d.key=="1"|| d.key=="11" || d.key=="16" || d.key=="23" ){//单行文本框、下拉选项、隐藏域
				objSelect.append(option);
			}
		}
	});
	...
	if(field.fieldType=='number'){
		...
		try{
			...
			if(property.isShowComdify == "1"){
				$("#isShowComdify").attr("checked",true);
			}
...
function hdlControlTypeShow(ft,vf,value){
	...
	case number_://数字
		...
		case '0'://表单输入
			...
			controlList[3] = {key : '23',value : '小数只读转百分比'};
		case '1'://脚本输入 (显示)
		case '2'://脚本输入 (隐藏)
			controlTypeTr.show();
...
function initOptionArray(json,ct) {
	...
	if (typeof(json)!=undefined && json!=null && json != '' && json.length>0){  //有数据时
		...
		if(ct=='11' || ct=='13'){//这里是修复了下拉框选项为null时的BUG by liyj
			return;
		}
