...
FormUtil.initEventBtn=function(scope){
	if(!scope){
		var eventBtn = $(".extend[eventBtn]");
	}else{
		var eventBtn  = $(".extend[eventBtn]",scope);
	}
...
FormUtil.handlEventBtnEvent = function(type,scope){
	...
	eventBtn.each(function(){
		...
		for(var i=0,queryJson;queryJson=queryArr[i++];){
			...
			targetInput.on("keyup change", function(event){
				//判断是否触发。 
				if(event.keyCode != 13 && event.type != 'change') return;
				var queryJsonArr = $(this).data("queryJsonArr");
				if(!queryJsonArr ||queryJsonArr.length<1) return;
				var linkMessage =queryJsonArr[0][0].linkMessage;
				
				if(linkMessage.linkType == "2" && event.type != 'change') return;
				if(linkMessage.linkType == "1" && event.keyCode != 13) return;
				/* 新等待处理完后加载查询
				 * 将待触发查询放在全局队列中，
				 * 过会执行这些查询。js单线程 防止对话框确定按钮等待触发查询的太长时间。呵呵！
				 * */
		...
		for(var j=0,target;target=targets[j++];){
			...
			//子表不添加行情况 
				if(isSubtable && !isAddRow){
					...
					if(parentObj.length <= 0) parentObj = me.closest(".blocktable");
					if(parentObj.length <= 0) parentObj = me.closest(".listTable");
					if(parentObj.length <= 0) parentObj = me.closest("[formtype=window]");//弹出框
				}
				else {
        ...
        6:function(c){//非必填
			...
			if(FormUtil.isAttachment(self)){
				...
				if(validate){
					...
					self.removeClass("validError");
					if(self.hasClass("Wdate")){
						self.css("border","1px solid #8cd5fa");
					}
				}
			}
			FormUtil.linkageShow(self);
	...
	$("body").delegate("a[linktype]","click",function(){
		...
		case 20:
			...
			$.ajax({
				...
				async : false,
				dataType : "json",
				url : __ctx + "/platform/bpm/task/getMyTaskByRunId.ht?runId="+runId,
			...
			if(taskId != 0){
				url =__ctx+"/bpm/platform/bpm/task/toStart.ht?taskId="+taskId;
			}
