...
OfficeControl = function() {
	...
	this.memuItems = {
		...
		items : [ {
			...
			this.getControlHtml = function(controlId) {
				var cabPath = __ctx
						+ "/media/office/OfficeControl.cab#version=5,0,2,7";
				...
				if (Sys.ie) {
					...
					if (this.config.doctype == 'pdf') {
						...
						str = '<object id="'
								...
								+ '" width="'
								+ this.width
								+ '"  type="application/ntko-plug" ForOnSaveAsOtherFormatToURL="saveMethodOnComplete" ForOndocumentopened="documentOpenedOnComplete'
								+ this.config.myNum
								+ '" ForOnAddTemplateFromURL="addTemplateOnComplete'
								+ this.config.myNum + '" ';
					}
			...
			this.renderTo = function(divContainerId, conf) {
				...
				if ($("#" + divContainerId + " div[name='menuBar']").ligerMenuBar) {// 明细页面不需要
					$("#" + divContainerId + " div[name='menuBar']")
							.ligerMenuBar({
								items : items
							});
				}
				...
				if (Sys.ie || Sys.firefox || Sys.chrome) {
					...
					// $.ligerDialog.warn('office控件只支持IE、Firefox和Chrome
					// 32位版的浏览器!');
	...
	this.addPDFSupport = function() {
		...
		var path = __ctx + "/platform/system/sysFile/getFileById.ht?fileId="
				+ this.config.fileId;
		this.controlObj.AddDocTypePlugin(".pdf", "PDF.NtkoDocument", "4.0.0.1",
				__ctx + "/media/office/ntkopdfdoc.cab", 51, true); // 引用pdf组件
